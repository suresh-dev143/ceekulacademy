...
export class DocumentUploadService {
  ...
  private readonly endpoint = `${environment.apiUrl}/api/upload/document`;

  uploadDocument(file: File): Observable<DocumentUploadProgress> {
    ...
    return this.http.post<DocumentUploadResponse>(this.endpoint, formData, {
      observe: 'events',
      reportProgress: true,
    }).pipe(
  ...
  private mapUploadEvent(
    event: HttpEvent<DocumentUploadResponse>,
  ...
  private normalizeUploadError(error: unknown): DocumentUploadValidationError {
    const message =
      typeof error === 'object' &&
      error !== null &&
      'error' in error &&
