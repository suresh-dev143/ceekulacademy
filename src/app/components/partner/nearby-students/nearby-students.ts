...
@Component({
      ...
      <div class="user-grid">
        <div class="user-card" *ngFor="let student of students">
          ...
          <div class="card-actions">
            <button class="btn-accent-sm" (click)="invite.emit(student.id)">Invite to Program</button>
            <button class="btn-ghost-sm">View Summary</button>
          </div>
        </div>
      </div>
  ...
  styles: [`
    ...
    .user-avatar.student { width: 44px; height: 44px; border-radius: 0; background: #000000; border: 1px solid var(--accent-primary); color: var(--accent-primary); display: flex; align-items: center; justify-content: center; font-weight: 900; font-size: 1.1rem; }
    .user-name { font-size: 1.1rem; font-weight: 800; color: #fff; margin: 0; text-transform: uppercase; letter-spacing: 0.5px; }
    .user-interest { font-size: 0.75rem; color: var(--text-secondary); font-weight: 700; line-height: 1.4; display: block; margin-top: 0.2rem; text-transform: uppercase; letter-spacing: 0.5px; }
    ...
    .btn-accent-sm { background: #000000; color: var(--text-primary); border: 1px solid var(--accent-primary); padding: 0.7rem; border-radius: 0; font-size: 0.8rem; font-weight: 900; cursor: pointer; text-transform: uppercase; letter-spacing: 1px; transition: 0.2s; &:hover { background: var(--accent-primary); color: #000000; } }
