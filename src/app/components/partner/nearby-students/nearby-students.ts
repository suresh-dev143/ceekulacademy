...
@Component({
        ...
        <div class="user-card" *ngFor="let student of students">
          ...
          <div class="info-list">
            <div class="info-item">
              ...
              <span>Active {{ student.activityType }}</span>
            </div>
            <div class="info-item">
          ...
          <div class="card-actions">
            <button class="btn-accent-sm" (click)="invite.emit(student.id)">Invite</button>
  ...
  styles: [`
    ...
    .section-title { font-size: 1rem; font-weight: 800; color: #fff; margin: 0; display: flex; align-items: center; gap: 0.6rem; text-transform: uppercase; letter-spacing: 1px; i { color: #10b981; } }
    .count-badge { font-size: 0.65rem; font-weight: 800; padding: 0.3rem 0.8rem; background: rgba(16, 185, 129, 0.15); border: 1px solid rgba(16, 185, 129, 0.3); color: #6ee7b7; border-radius: 20px; text-transform: uppercase; letter-spacing: 1px; }
    ...
    .dist-tag { position: absolute; top: 1rem; right: 1rem; font-size: 0.6rem; font-weight: 800; color: #3b82f6; background: rgba(59, 130, 246, 0.1); text-transform: uppercase; border: 1px solid rgba(59, 130, 246, 0.3); padding: 0.25rem 0.6rem; border-radius: 12px; letter-spacing: 0.5px; }

    .card-header { display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem; }
    .user-avatar.student { width: 44px; height: 44px; border-radius: 12px; background: linear-gradient(135deg, #10b981, #059669); border: 1px solid rgba(255,255,255,0.1); color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 1.2rem; }
    .user-name { font-size: 1rem; font-weight: 800; color: #fff; margin: 0; text-transform: uppercase; letter-spacing: 0.5px; }
    .user-interest { font-size: 0.75rem; color: #9ca3af; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; display: block; margin-top: 0.2rem; }

    .info-list { display: flex; flex-direction: column; gap: 0.6rem; margin-bottom: 1.25rem; }
    .info-item { display: flex; align-items: center; gap: 0.8rem; font-size: 0.75rem; color: #cbd5e1; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; i { color: #10b981; width: 14px; text-align: center; } }

    .card-actions { display: grid; grid-template-columns: 1fr 1fr; gap: 0.6rem; }
    .btn-accent-sm { background: linear-gradient(135deg, #34d399, #10b981); color: #000; border: none; padding: 0.7rem; border-radius: 10px; font-size: 0.75rem; font-weight: 800; cursor: pointer; text-transform: uppercase; letter-spacing: 0.5px; transition: transform 0.2s, box-shadow 0.2s; &:hover { transform: translateY(-1px); box-shadow: 0 4px 12px rgba(16, 185, 129, 0.4); } }
...
export class NearbyStudentsComponent {
  @Input() students: NearbyUser[] = [];
